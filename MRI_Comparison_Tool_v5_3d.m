...
% MRI_Comparison_Tool_v5 - Compare ground truth and motion-corrected MRI data
%
% New in v5:
%   - Clear buttons (✕) next to Load GT and Load MC to unload data independently
%   - Clearing one dataset resets its axes, label, and metrics cleanly
%
% New in v4:
...
% Usage: Run MRI_Comparison_Tool_v4() in MATLAB command window.
    ...
    %% ---- Create Main Figure ----
    fig = figure('Name', 'MRI Comparison Tool v5', ...
        'NumberTitle', 'off', ...
        'Units', 'normalized', ...
    ...
    uicontrol(top_panel, 'Style', 'pushbutton', ...
        'String', char(10005), ...           % ✕
        'Units', 'normalized', ...
        'Position', [0.135 0.15 0.03 0.7], ...
        'FontSize', 11, 'FontWeight', 'bold', ...
        'BackgroundColor', [0.65 0.15 0.15], ...
        'ForegroundColor', 'w', ...
        'TooltipString', 'Clear Ground Truth data', ...
        'Callback', @(~,~) clearData('gt'));

    state.gt_label = uicontrol(top_panel, 'Style', 'text', ...
        'String', 'No file loaded', ...
        'Units', 'normalized', ...
        'Position', [0.17 0.15 0.22 0.7], ...
        'FontSize', 9, ...
        'BackgroundColor', [0.2 0.2 0.2], ...
        'ForegroundColor', [0.8 0.8 0.8], ...
        'HorizontalAlignment', 'left');

    % --- Motion Corrected: Load + Clear + Label ---
    uicontrol(top_panel, 'Style', 'pushbutton', ...
        'String', 'Load Motion Corrected', ...
        'Units', 'normalized', ...
        'Position', [0.40 0.15 0.14 0.7], ...
        'FontSize', 10, 'FontWeight', 'bold', ...
    ...
    uicontrol(top_panel, 'Style', 'pushbutton', ...
        'String', char(10005), ...           % ✕
        'Units', 'normalized', ...
        'Position', [0.545 0.15 0.03 0.7], ...
        'FontSize', 11, 'FontWeight', 'bold', ...
        'BackgroundColor', [0.65 0.15 0.15], ...
        'ForegroundColor', 'w', ...
        'TooltipString', 'Clear Motion Corrected data', ...
        'Callback', @(~,~) clearData('mc'));

    state.mc_label = uicontrol(top_panel, 'Style', 'text', ...
        'String', 'No file loaded', ...
        'Units', 'normalized', ...
        'Position', [0.58 0.15 0.22 0.7], ...
        'FontSize', 9, ...
        'BackgroundColor', [0.2 0.2 0.2], ...
        'ForegroundColor', [0.8 0.8 0.8], ...
        'HorizontalAlignment', 'left');
    ...
    function clearData(type)
        switch type
            case 'gt'
                state.gt_data   = [];
                state.gt_name   = 'Ground Truth';
                set(state.gt_label, 'String', 'No file loaded');
                % Blank the GT axes
                cla(state.ax_gt);
                title(state.ax_gt, 'Ground Truth', 'Color', 'w', ...
                    'FontSize', 12, 'FontWeight', 'bold');
                axis(state.ax_gt, 'image', 'off');
            case 'mc'
                state.mc_data   = [];
                state.mc_name   = 'Motion Corrected';
                set(state.mc_label, 'String', 'No file loaded');
                % Blank the MC and Diff axes (diff needs both datasets)
                cla(state.ax_mc);
                title(state.ax_mc, 'Motion Corrected', 'Color', 'w', ...
                    'FontSize', 12, 'FontWeight', 'bold');
                axis(state.ax_mc, 'image', 'off');
                cla(state.ax_diff);
                title(state.ax_diff, 'Difference', 'Color', 'w', ...
                    'FontSize', 12, 'FontWeight', 'bold');
                axis(state.ax_diff, 'image', 'off');
        end

        % Both datasets must be present for pair-wise operations
        state.data_loaded = ~isempty(state.gt_data) && ~isempty(state.mc_data);

        % Clear metrics and profile when either dataset is gone
        set(state.metrics_text, 'String', 'Load both datasets to see metrics');
        set(state.range_text,   'String', '');
        cla(state.ax_profile);
        title(state.ax_profile, 'Line Profile (middle row)', 'Color', 'w', 'FontSize', 10);
        xlabel(state.ax_profile, 'Column',    'Color', 'w');
        ylabel(state.ax_profile, 'Intensity', 'Color', 'w');
        grid(state.ax_profile, 'on');

        % If the remaining dataset is GT-only, still render it alone
        updateDisplay();
        fprintf('Cleared %s data.\n', upper(type));
    end
